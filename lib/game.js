...
var createPlayers = function (game) {
  game.players = [];
  var grid = 10;
...
var logPositions = function (game, players) {
  players.forEach(function(player) {
    game.occupiedPositions[player.x] = game.occupiedPositions[player.y] || [];
...
function checkDeaths(players){
  players.forEach(function(player) {
    if (player.died()) { console.log('collision!!! at '+ player.position().toString()); }
  });
}
