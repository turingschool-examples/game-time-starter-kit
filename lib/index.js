...
function oneSecondFunction() {
  if (score.length > 10 &&  score.length < 20) {
    gameSpeed = 1
    ballSpeed = 7
  }
  else if (score.length > 20 && score.length < 30){
    gameSpeed = 2
    ballSpeed = 8
  }
  else if (score.length > 30 && score.length < 40){
    gameSpeed = 3
    ballSpeed = 9
  }
  else if (score.length > 40){
    gameSpeed = 4
    ballSpeed = 10
  }
}
...
requestAnimationFrame(function gameLoop(){
  ...
  if (gameInPlay && !lost) {
    ...
    else if (score.length > 5 && score.length < 10) {
      gameSpeed = 0.5
    }
