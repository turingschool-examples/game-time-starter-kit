var highScores = JSON.parse(localStorage["high-scores"]) || []
displayHighScores()
...
function drawGame(ctx, fallSpeed, acceleration, ballSpeed, canvas, gameInPlay, gates, ball, adjustedScore) {
  // localStorage.high-scores = "Smith";
  // document.getElementById("high-score-list").innerHTML = localStorage.lastname;
  ball.draw(ctx)
...
function resetGame(ball, canvas, game) {
  ...
  gates = []
  GateBuilder(gates, canvas)
  if (scoreAdded === false) { highScores.push(game.score); }
  highScores.sort(function(a, b){return b-a});
  if (highScores.length > 5) {
    highScores.pop();
  }
  // localStorage["high-scores"] = JSON.stringify(highScores);
  // let storedScores = JSON.parse(localStorage["high-scores"]);
  if (scoreAdded === false) {
    displayHighScores()
  }
  scoreAdded = true;
