...
var frog = new Frog(375, 550, width, height);
...
requestAnimationFrame(function gameLoop() {
  ...
  car4.forEach(function (car) {car.carDraw(context).carMove(); collision(car)});

  log1.forEach(function (log) {log.logDraw(context).logMove(); rideL(log)});
  log2.forEach(function (log) {log.logDraw(context).logMove(); rideL(log)});
  turtle1.forEach(function(turtle) {turtle.turtleDraw(context).turtleMove(); rideT(turtle)});
  turtle2.forEach(function(turtle) {turtle.turtleDraw(context).turtleMove(); rideT(turtle)});
